#!/usr/bin/env python3
"""
Same correlation diagnostic as diagnose_correlation_sift128.py -- does each
scoring scheme correlate with the true per-query minimum required EF -- run
on dbpedia-openai-1000k-angular (1536-dim OpenAI ada-002 text embeddings;
see benchmark_dbpedia_openai1536.py for why this dataset was chosen).

Reuses benchmark_dbpedia_openai1536.py's exact cached HNSW index and
K-Means/bin caches. Run that script first (or at least let it build the
index and K=1/50/100/200 cluster caches) to avoid rebuilding here too.
"""
...
LOG_PATH = f"diagnose_correlation_dbpedia_openai1536_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
gt_path = f"dbpedia_openai1536_calib_gt_{N_CALIB}q_diag.npz"
...
if os.path.exists(index_path):
    ...
    print("No cached index found -- building (run benchmark_dbpedia_openai1536.py "
...
minef_cache = f"dbpedia_openai1536_calib_min_ef_{N_CALIB}q_diag.npz"
...
print(f"\n{'=' * 60}")
print("  SUMMARY (dbpedia-openai-1536): correlation between score and true required-EF")
print(f"{'=' * 60}")
for name, rho, p in results:
    print(f"  {name:<30} rho={rho:+.4f}")
print("\nCompare against diagnose_anisotropy.py --dataset dbpedia_openai1536's KS-fit number")
print("and the established table (worst->best fit: SIFT 0.125, DeepImage 0.067, MS MARCO")
print("0.047, Cohere 0.043, LAION 0.029, GloVe 0.016).")
