...
//                    if (candidate_id == 0) continue;
    ...
    std::priority_queue<std::pair<dist_t, labeltype>>
    searchKnnAdaptive(const void *query_data, size_t k, tableint start_node, int start_layer, size_t ef) const {
        std::priority_queue<std::pair<dist_t, labeltype>> result;
        if (cur_element_count == 0 || start_node >= cur_element_count) return result;

        start_layer = std::max(0, std::min(start_layer, element_levels_[start_node]));

        tableint currObj = start_node;
        dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(start_node), dist_func_param_);

        for (int level = start_layer; level > 0; level--) {
            bool changed = true;
            while (changed) {
                changed = false;
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
        top_candidates = searchBaseLayerST<true, true>(currObj, query_data, std::max(ef, k));
    ...
    std::priority_queue<std::pair<dist_t, labeltype>>
    searchKnnDynamic(const void *query_data, size_t k, const std::vector<float>& bins, const std::vector<int>& ef_table, int min_ef, int max_ef, int probe_count) const {
        std::priority_queue<std::pair<dist_t, labeltype>> result;
        if (cur_element_count == 0) return result;
        ...
        while (!candidate_set.empty()) {
            ...
            if (candidate_dist > lowerBound) {
                if (!ef_updated) {
                    ...
                    float score = 0;
                    for (float d : top_dists) {
                        for (float b : bins) {
                            if (d > b) score += 1.0f;
                        }
                    }
                    int score_idx = std::max(0, (int)score);
    ...
    float
    getDynamicProbeScore(const void *query_data, const std::vector<float>& bins, int probe_count) const {
    ...
    // -------------------------------------------------------------------------
    // Paper-exact scoring: first-matching-bin assignment + exponential-decay
    // weights, mirroring hnsw-ada-ef's ApproximatedScoreCalculator::calculate_score
    ...
    // design as searchKnnDynamic, but using the paper-exact scoring above, and
    // returning the ef actually used so callers can report it.
    std::pair<std::priority_queue<std::pair<dist_t, labeltype>>, int>
    searchKnnDynamicWeighted(const void *query_data, size_t k, const std::vector<float>& bins, const std::vector<float>& weights, const std::vector<int>& ef_table, int min_ef, int max_ef, int probe_count) const {
        std::priority_queue<std::pair<dist_t, labeltype>> result;
        if (cur_element_count == 0) return {result, min_ef};
        ...
        return {result, current_ef};
    }

    // Offline-calibration counterpart of searchKnnDynamicWeighted: computes the
    // paper-exact score for a probe_count-sized traversal without continuing to
    // a full search. Used only to bucket calibration queries; not counted toward
    // online distance-computation totals (matches how calibration is treated
    // for both the Ada-ef and cluster-aware methods).
    float
    getDynamicProbeScoreWeighted(const void *query_data, const std::vector<float>& bins, const std::vector<float>& weights, int probe_count) const {
        if (cur_element_count == 0) return 0.0f;
    ...
    std::priority_queue<std::pair<dist_t, labeltype>>
    searchKnnTrueAda(const void *query_data, size_t k, const std::vector<float>& bins, const std::vector<float>& weights, const std::vector<int>& ef_table, int min_ef, int max_ef, int probe_count) const {
        std::priority_queue<std::pair<dist_t, labeltype>> result;
        if (cur_element_count == 0) return result;

        float score = 0;
        int count = 0;
        tableint ep_global = enterpoint_node_;
        
        int *data_ep = (int *) get_linklist0(ep_global);
        size_t size_ep = getListCount((linklistsizeint*)data_ep);
        
        for (size_t j = 1; j <= size_ep; j++) {
            tableint cand = *(data_ep + j);
            dist_t nd = fstdistfunc_(query_data, getDataByInternalId(cand), dist_func_param_);
        ...
        int int_score = (int)std::round(score);
        int pred_ef = max_ef;
        if (int_score >= 0) {
            if (int_score < (int)ef_table.size()) {
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
        top_candidates = searchBaseLayerST<true, true>(currObj, query_data, std::max((size_t)current_ef, k));
    ...
    std::vector<std::tuple<int, labeltype, dist_t>> profileQuery(const void *query_data) const {
        std::vector<std::tuple<int, labeltype, dist_t>> path;
        if (cur_element_count == 0) return path;
        ...
        for (int level = maxlevel_; level >= 0; level--) {
            ...
            path.emplace_back(level, getExternalLabel(currObj), curdist);
        }
        return path;
    }

    size_t get_dist_count() const { return metric_distance_computations; }
    void reset_dist_count() const { metric_distance_computations = 0; }
