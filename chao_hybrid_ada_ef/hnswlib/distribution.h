...
#pragma omp for schedule(static)
    ...
    private:
        ...
        double normal_quantile(double p) {
            ...
            double d1 = 3.54388920042, d2 = 1.63706780097;
            double r, x, y=p-0.5;
            if(std::abs(y)<0.42) {
                r = y*y;
        ...
        void initialize_threshold_z()
        {
            ...
            if (isTopBased)
            {
                for (int i = 0; i < num_bins; ++i)
                {
                    threshold_z[i] = normal_quantile((1 - quantile_step * num_bins) + quantile_step * i);
                }
            }
            else
            {
                for (int i = 0; i < num_bins; ++i)
                {
                    threshold_z[i] = normal_quantile(quantile_step * (i + 1));
                }
            }
        }
            ...
            const float mean, const float var) const
        {
            ...
            // Calculate bin intervals
            std::vector<float> bin_thresholds(num_bins);
            for (int i = 0; i < num_bins; ++i)
            {
