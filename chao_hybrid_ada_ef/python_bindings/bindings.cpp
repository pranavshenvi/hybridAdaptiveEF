...
PYBIND11_PLUGIN(chao_hybrid_ada_ef_cpp) {
        ...
        .def_property_readonly("entry_point", [](const Index<float> & index) {
          return index.index_inited ? index.appr_alg->enterpoint_node_ : 0;
        })
        .def_property_readonly("max_level", [](const Index<float> & index) {
          return index.index_inited ? index.appr_alg->maxlevel_ : 0;
        })
        .def("get_dist_count", [](const Index<float> & index) {
          return index.index_inited ? index.appr_alg->get_dist_count() : 0;
        })
        .def("reset_dist_count", [](Index<float> & index) {
          if (index.index_inited) index.appr_alg->reset_dist_count();
        })
        .def("search_knn_adaptive", [](Index<float>& self, py::array_t<float, py::array::c_style> query, int k, int entry_node, int start_layer, int ef) {
            auto buf = query.request();
            if (buf.ndim != 1) throw std::runtime_error("Expected 1-D query vector");
            auto res = self.appr_alg->searchKnnAdaptive(static_cast<const float*>(buf.ptr), k, entry_node, start_layer, ef);
        ...
        .def("search_knn_dynamic", [](Index<float>& self, py::array_t<float, py::array::c_style> query, int k, std::vector<float> bins, std::vector<int> ef_table, int min_ef, int max_ef, int probe_count) {
            auto buf = query.request();
            if (buf.ndim != 1) throw std::runtime_error("Expected 1-D query vector");
            auto res = self.appr_alg->searchKnnDynamic(static_cast<const float*>(buf.ptr), k, bins, ef_table, min_ef, max_ef, probe_count);
            py::array_t<hnswlib::labeltype> labels(k);
            py::array_t<float> dists(k);
            auto lb = labels.mutable_unchecked<1>();
            auto db = dists.mutable_unchecked<1>();
        ...
        .def("get_dynamic_probe_score", [](Index<float>& self, py::array_t<float, py::array::c_style> query, std::vector<float> bins, int probe_count) {
            auto buf = query.request();
            if (buf.ndim != 1) throw std::runtime_error("Expected 1-D query vector");
        ...
        .def("search_knn_true_ada", [](Index<float>& self, py::array_t<float, py::array::c_style> query, int k, std::vector<float> bins, std::vector<float> weights, std::vector<int> ef_table, int min_ef, int max_ef, int probe_count) {
            auto buf = query.request();
            if (buf.ndim != 1) throw std::runtime_error("Expected 1-D query vector");
            auto res = self.appr_alg->searchKnnTrueAda(static_cast<const float*>(buf.ptr), k, bins, weights, ef_table, min_ef, max_ef, probe_count);
            py::array_t<hnswlib::labeltype> labels(k);
            py::array_t<float> dists(k);
            auto lb = labels.mutable_unchecked<1>();
            auto db = dists.mutable_unchecked<1>();
        ...
        .def("profile_query", [](Index<float>& self, py::array_t<float, py::array::c_style> query) {
            auto buf = query.request();
            if (buf.ndim != 1) throw std::runtime_error("Expected 1-D query vector");
            auto path = self.appr_alg->profileQuery(static_cast<const float*>(buf.ptr));
            py::list ret;
            for (auto& p : path) {
                ret.append(py::make_tuple(std::get<0>(p), std::get<1>(p), std::get<2>(p)));
            }
            return ret;
        })
