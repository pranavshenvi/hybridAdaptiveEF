...
int search_from_candidates_DARTH(
  ...
  float prev_recall = 0;
  int total_insertions = 0;
  int prediction_interval = darth_predictor.initial_prediction_interval;
  ...
  while (candidates.size() > 0) {
      ...
      if (counter == 4) {
        ...
        for (size_t id4 = 0; id4 < 4; id4++) {
          ...
          early_terminate = add_to_heap(
              saved_j[id4], dis[id4], &predicted_recall, &prediction_interval, &query_predictor_calls, &predictor_time);
          if (early_terminate) {
            break;
          }
        }
    ...
    for (size_t icnt = 0; icnt < counter; icnt++) {
      ...
      ndis++;
      ndis_interval++;
      early_terminate = add_to_heap(saved_j[icnt], dis, &predicted_recall, &prediction_interval, &query_predictor_calls, &predictor_time);
      if (early_terminate) {
        break;
      }
    }
