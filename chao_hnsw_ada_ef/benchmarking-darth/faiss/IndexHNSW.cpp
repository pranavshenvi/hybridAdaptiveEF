...
void hnsw_add_vertices(
  ...
  if (verbose) {
    printf(
        "hnsw_add_vertices: adding %zd elements on top of %zd "
        "(preset_levels=%d)\n",
        n,
        n0,
        int(preset_levels));
...
template <class BlockResultHandler>
void hnsw_search_declarative_recall_data_generation(
    const IndexHNSW* index,
    idx_t n,
    const float* x,
    BlockResultHandler& bres,
  ...
  idx_t query_batch_size = 100;
  idx_t total_batches = (n + query_batch_size - 1) / query_batch_size;

  // Print batch information
  std::cout << ">> query_batch_size: " << query_batch_size << std::endl;
  std::cout << ">> total_batches: " << total_batches << std::endl;

  std::string query_monitor_data[query_batch_size];

  // Loop through each batch
  for (idx_t current_batch = 0; current_batch < total_batches;
       current_batch++) {
    idx_t i0 = current_batch * query_batch_size;
    idx_t i1 = std::min(i0 + query_batch_size, n);

    std::cout << ">> Answering queries " << i0 << " to " << i1 - 1
              << std::endl;

    for (idx_t i = 0; i < query_batch_size; i++) {
      query_monitor_data[i].clear();
      query_monitor_data[i].shrink_to_fit();
    }

#pragma omp parallel
    {
      VisitedTable vt(
          index->ntotal);  // Each thread gets its own VisitedTable
      typename BlockResultHandler::SingleResultHandler res(bres);
...
#pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
      for (idx_t i = i0; i < i1; i++) {
        ...
        idx_t query_id_for_table = i - i0;
        HNSWStats stats = hnsw.search_declarative_recall_data_generation(
            *dis,
            res,
            vt,
            i,
        ...
        // printf("Actual query id: %ld, query id for table: %ld\n", i,
        // query_id_for_table); query_monitor_data[query_id_for_table] =
        // "Query " + std::to_string(i) + ":\n";
        res.end();
      }
    }

    data_collector.flush_all_observations_to_log(
        query_monitor_data, query_batch_size);
...
void hnsw_search_baseline(
  ...
  for (idx_t i0 = 0; i0 < n; i0 += check_period) {
    ...
    // #pragma omp parallel
    {
      ...
      // #pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
      for (idx_t i = i0; i < i1; i++) {
        // printf(">> Answering query %ld\n", i);
        res.begin(i);
        dis->set_query(x + i * index->d);

        HNSWStats stats = hnsw.search_baseline(
            *dis, res, vt, i, data_collector, params);
        n1 += stats.n1;
        n2 += stats.n2;
        ndis += stats.ndis;
        res.end();
  ...
  for (idx_t i0 = 0; i0 < n; i0 += check_period) {
    ...
    // #pragma omp parallel
    {
      ...
      for (idx_t i = i0; i < i1; i++) {
        ...
        HNSWStats stats = hnsw.search_LAET(
            *dis, res, vt, i, laet_predictor, params);
        n1 += stats.n1;
        n2 += stats.n2;
        ndis += stats.ndis;
        res.end();
...
template <class BlockResultHandler>
void hnsw_search_optimal_early_stop(
    const IndexHNSW* index,
    idx_t n,
    const float* x,
    BlockResultHandler& bres,
  ...
  for (idx_t i0 = 0; i0 < n; i0 += check_period) {
    ...
    // #pragma omp parallel
    {
      ...
      for (idx_t i = i0; i < i1; i++) {
        ...
        HNSWStats stats = hnsw.search_optimal_early_stop(
            *dis, res, vt, i, data_collector, params);
        n1 += stats.n1;
        n2 += stats.n2;
        ndis += stats.ndis;
  ...
  for (idx_t i0 = 0; i0 < n; i0 += check_period) {
    ...
    // #pragma omp parallel
    {
      ...
      for (idx_t i = i0; i < i1; i++) {
        ...
        HNSWStats stats = hnsw.search_DARTH(*dis, res, vt, i, darth_predictor, params);
        n1 += stats.n1;
        n2 += stats.n2;
        ndis += stats.ndis;
        res.end();
...
void IndexHNSW::search_declarative_recall_data_generation(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
  ...
  hnsw_search_declarative_recall_data_generation(this, n, x, bres, data_collector, params_in);
...
void IndexHNSW::search_baseline(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
  ...
  hnsw_search_baseline(this, n, x, bres, data_collector, params_in);
...
void IndexHNSW::search_DARTH(
  ...
  hnsw_search_DARTH(this, n, x, bres, darth_predictor, params_in);
...
void IndexHNSW::search_LAET(
  ...
  hnsw_search_LAET(this, n, x, bres, laet_predictor, params_in);
