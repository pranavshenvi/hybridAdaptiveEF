...
void IndexIVF::search_generate_training_data(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
    idx_t* labels,
    DeclarativeRecallDataCollectorIVF searchMonitor,
    const SearchParameters* params_in) const {
  FAISS_THROW_IF_NOT(k > 0);
  const IVFSearchParameters* params = nullptr;
  if (params_in) {
  ...
  auto sub_search_func = [this, k, nprobe, params](
    ...
    search_preassigned_training_data_generation(
        n,
        x,
        k,
        idx.get(),
  ...
  sub_search_func(n, x, distances, labels, &indexIVF_stats, searchMonitor);  // only for the testing

  searchMonitor.close_log_file();
  return;  // pay attention here!!
...
#pragma omp parallel for if (nt > 1)
    ...
    // handle parallelization at level below (or don't run in parallel at
    // all)
    sub_search_func(n, x, distances, labels, &indexIVF_stats, searchMonitor);
  }

  searchMonitor.close_log_file();
}

void IndexIVF::search_plainIVF(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
...
void IndexIVF::search_DARTH(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
    idx_t* labels,
    DARTHPredictorIVF darth_predictor,
    const SearchParameters* params_in) const {
  FAISS_THROW_IF_NOT(k > 0);
  const IVFSearchParameters* params = nullptr;
  if (params_in) {
  ...
  printf("Parallel mode: %d\n", parallel_mode);
  sub_search_func(n, x, distances, labels, &indexIVF_stats, darth_predictor);  // only for the testing

  darth_predictor.close_log_file();
  return;  // pay attention here!!
...
void IndexIVF::search_preassigned_training_data_generation(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* keys,
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap)
  {
    ...
    auto scan_one_list = [&](idx_t key,
                             ...
                             idx_t qid,
                             size_t nprobe_id,
                             float q_first_centroid_dist,
                             double q_search_start_time,
                             DeclarativeRecallDataCollectorIVF searchMonitor,
                             size_t* q_ndis,
                             size_t* q_ninserts) {
      if (key < 0) {
        ...
        if (invlists->use_iterator) {
          ...
          if (!scanner->keep_max) {
            for (size_t j = 0; j < list_size; j++) {
              ...
              if (dis < simi[0]) {
                int64_t id = store_pairs ? lo_build(scanner->list_no, j) : ids[j];
              ...
              if (*q_ninserts >= searchMonitor.dataManager.k) {
                if (*q_ndis % searchMonitor.logging_interval == 0) {
                  float current_recall = searchMonitor.dataManager.get_recallk(qid);
                  double q_elapsed_time = searchMonitor.dataManager.elapsed_secs() - q_search_start_time;
                  searchMonitor.append_to_log(qid, nprobe_id, *q_ndis, *q_ninserts, q_elapsed_time, q_first_centroid_dist, current_recall);
                }
              }
            }
          } else {
...
#pragma omp for
      ...
      for (idx_t i = 0; i < n; i++) {
        ...
        double q_search_start_time = searchMonitor.dataManager.elapsed_secs();
        // todo: what is the distance of the query to the first centroid?
        float first_centroid_dist = 0.0;
        ...
        for (size_t ik = 0; ik < nprobe; ik++) {
          if (ik == 0) {
            // idx_t centroid_id = keys[i * nprobe + ik];
            //
            // const float* centroid = NULL;//quantizer->xb.?? + centroid_id * d;
            // const float* query = x + i * searchMonitor.dataManager.d;
            //
            // first_centroid_dist = searchMonitor.dataManager.get_ed(query, centroid, searchMonitor.dataManager.d);
            first_centroid_dist = coarse_dis[i * nprobe + ik];
          }
        ...
        double q_final_time = searchMonitor.dataManager.elapsed_secs() - q_search_start_time;
        float q_final_recall = searchMonitor.dataManager.get_recallk(qid);
        printf("[Query %ld] recall: %f, ndc: %ld, ninserts: %ld, final-time: %lf\n", qid, q_final_recall, q_ndis, q_ninserts, q_final_time);
...
void IndexIVF::search_preassigned_plainIVF(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* keys,
...
#pragma omp for
      ...
      for (idx_t i = 0; i < n; i++) {
        ...
        // query has finalized processing:
        double q_final_time = searchMonitor.dataManager.elapsed_secs() - q_search_start_time;
        float q_final_recall = searchMonitor.dataManager.get_recallk(qid);
        printf("[Query %ld] recall: %f, ndc: %ld, ninserts: %ld, final-time: %lf\n", qid, q_final_recall, q_ndis, q_ninserts, q_final_time);
        searchMonitor.append_to_log(qid, nprobe, q_ndis, q_ninserts, q_final_time, first_centroid_dist, q_final_recall);
...
void IndexIVF::search_preassigned_DARTH(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* keys,
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap)
  {
    ...
    auto scan_one_list = [&](idx_t key,
                             ...
                             float q_first_centroid_dist,
                             double q_search_start_time,
                             DARTHPredictorIVF darthPredictor,
                             size_t* q_ndis,
                             size_t* q_ninserts,
                             size_t* q_ndis_interval,
                             float* predicted_recall_out,
                             bool* early_termination_triggered,
        ...
        if (invlists->use_iterator) {
          ...
          if (!scanner->keep_max) {
            for (size_t j = 0; j < list_size; j++) {
              ...
              if ((*q_ndis_interval) % *prediction_interval == 0) {
                double q_elapsed_time = darthPredictor.dataManager.elapsed_secs() - q_search_start_time;
                *predicted_recall_out = darthPredictor.predictRecall(
                    qid,
                    nprobe_id,
                    *q_ndis,
                    *q_ninserts,
                    q_elapsed_time,
                    q_first_centroid_dist,
                    *query_predictor_calls,
                    prediction_interval,
                    predictor_time);

                if (*predicted_recall_out >= darthPredictor.target_recall) {
...
#pragma omp for
      ...
      for (idx_t i = 0; i < n; i++) {
        ...
        scanner->set_query(x + i * d);
        // darthPredictor.prepare_for_next_query();
        ...
        size_t q_ndis_interval = 0;

        double q_search_start_time = darth_predictor.dataManager.elapsed_secs();
        float first_centroid_dist = 0.0;
        int prediction_interval = 0;
        int query_predictor_calls = 0;
        ...
        float predicted_target_recall = 0.0;
        bool early_termination_triggered = false;

        // loop over probes
        size_t q_nprobe_id = 0;
        for (size_t ik = 0; ik < nprobe; ik++) {
          if (ik == 0) {
            first_centroid_dist = coarse_dis[i * nprobe + ik];
          }
        ...
        // log information!
        double q_elapsed_time = darth_predictor.dataManager.elapsed_secs() - q_search_start_time;
        float actual_recall = darth_predictor.dataManager.get_recallk(qid);
        printf("[Query %ld] => Ract: %f, Rp: %f, time: %lf, ndc: %ld, ninserts: %ld, nprobes: %ld, firstdist: %f, predictor calls: %d\n", qid, actual_recall, predicted_target_recall, q_elapsed_time, q_ndis, q_ninserts, q_nprobe_id, first_centroid_dist, query_predictor_calls);
...
size_t InvertedListScanner::scan_codes(
  ...
  // qid, time, current_recall, [cluster_num (nstep), ndis, ninserts], [dist_to_first_cluster, heap_nn_dist, heap_furtherst_dist, heap_dist_stats] || [dist2ndcluster, dist3rdcluster, ...]
