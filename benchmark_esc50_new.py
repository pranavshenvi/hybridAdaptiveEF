...
Dataset: ESC50 Audio Embeddings (2000 samples, 512d)
"""
...
RESULTS_DIR = f"results_esc50_{TIMESTAMP}"
os.makedirs(RESULTS_DIR, exist_ok=True)

sys.stdout.reconfigure(encoding='utf-8')

class Logger(object):
    def __init__(self, filename=os.path.join(RESULTS_DIR, "benchmark_esc50_sweep.log")):
...
def compute_ground_truth(samp_vecs, queries, k=10):
    ...
    gt = np.argpartition(probe_dists, k - 1, axis=1)[:, :k]
    for i in range(len(queries)):
        idx = np.argsort(probe_dists[i, gt[i]])
        gt[i] = gt[i][idx]
    return gt
...
print("═" * 80)
with h5py.File('esc50_embeddings.h5', 'r') as f:
    all_data = f['embeddings'][:].astype(np.float32)

indices = np.arange(len(all_data))
np.random.shuffle(indices)

corpus_idx = indices[:1600]
calib_idx = indices[1600:1800]
test_idx = indices[1800:2000]

corpus = all_data[corpus_idx]
calib_q = all_data[calib_idx]
test_q = all_data[test_idx]
dim = corpus.shape[1]
...
for K_CLUSTERS in K_SWEEP:
    ...
    print(f"{'─' * 80}")
    cache_file = f"kmeans_cache_k{K_CLUSTERS}_esc50.pkl"
    if os.path.exists(cache_file):
        ...
        km = MiniBatchKMeans(n_clusters=K_CLUSTERS, random_state=42, n_init=3, batch_size=400)
        km.fit(corpus)
