#!/usr/bin/env python3
"""
Benchmark: Cluster-Specific Ada-ef Architecture
...
RESULTS_DIR = f"results_local_{TIMESTAMP}"
os.makedirs(RESULTS_DIR, exist_ok=True)

sys.stdout.reconfigure(encoding='utf-8')

class Logger(object):
    def __init__(self, filename=os.path.join(RESULTS_DIR, "benchmark_cluster_specific.log")):
...
def build_full_list(table):
    # Builds an array indexing score 0 to 100 for instant lookup
...
if os.path.exists(calib_min_ef_path):
    print(f"  [CACHE] Loading cached calibration min-EF array from {calib_min_ef_path}...")
    calib_min_ef = np.load(calib_min_ef_path)
else:
    print(f"  [COMPUTE] Brute forcing required EF for {N_CALIB} calibration queries...")
    t0 = time.time()
    ...
    print(f"  Done in {time.time() - t0:.1f}s")
    print(f"  Saving calibration array to {calib_min_ef_path}")
    np.save(calib_min_ef_path, calib_min_ef)
...
def eval_cluster_aware_local(name, K_VAL, centroids, cluster_bins, global_list, local_lists):
    idx.reset_dist_count()
    recs, efs = [], []
    t0 = time.time()
    t_s = time.time()
    ...
    for i in range(n_test):
        ...
        # Determine if we should use local or global table
        local_table = local_lists[k_id]
        if not local_table:
            # Cluster was completely empty during training
            ef_used = global_list[min(score, len(global_list) - 1)]
            fallback_count += 1
        else:
            ef_used = local_table[min(score, len(local_table) - 1)]
    ...
    print(f"    (Used global fallback {fallback_count}/{n_test} times)")
    return dict(name=name, mean_r=np.mean(r), p5=np.percentile(r, 5), p1=np.percentile(r, 1),
...
for K_CLUSTERS in K_SWEEP:
    print(f"\n{'─' * 80}")
    print(f"  Cluster-Aware LOCAL TABLES with K={K_CLUSTERS}")
    print(f"{'─' * 80}")
    ...
    # -------------------------------------------------------------------
    # 1. Build Global Tables (For Fallback)
    # -------------------------------------------------------------------
    clust_table_mean_global = build_ef_table_mean(clust_calib_int, calib_min_ef)
    clust_table_p90_global = build_ef_table_p90(clust_calib_int, calib_min_ef)
    clust_table_p70_global = build_ef_table_p70(clust_calib_int, calib_min_ef)
    
    global_list_mean = build_full_list(clust_table_mean_global)
    global_list_p90 = build_full_list(clust_table_p90_global)
    global_list_p70 = build_full_list(clust_table_p70_global)

    # -------------------------------------------------------------------
    # 2. Build Local Tables per Cluster
    # -------------------------------------------------------------------
    local_lists_mean = [[] for _ in range(K_CLUSTERS)]
    local_lists_p90 = [[] for _ in range(K_CLUSTERS)]
    local_lists_p70 = [[] for _ in range(K_CLUSTERS)]
    
    local_tables_dump = {}
    empty_clusters = 0
    for k in range(K_CLUSTERS):
        mask = (calib_nearest == k)
        if np.sum(mask) == 0:
            empty_clusters += 1
            continue
            
        local_scores = clust_calib_int[mask]
        local_efs = calib_min_ef[mask]
        
        table_mean = build_ef_table_mean(local_scores, local_efs)
        table_p90 = build_ef_table_p90(local_scores, local_efs)
        table_p70 = build_ef_table_p70(local_scores, local_efs)
        
        local_lists_mean[k] = build_full_list(table_mean)
        local_lists_p90[k] = build_full_list(table_p90)
        local_lists_p70[k] = build_full_list(table_p70)
        
        local_tables_dump[f"cluster_{k}"] = {
            "mean": table_mean,
            "p90": table_p90,
            "p70": table_p70
        }
        
    print(f"  [INFO] {K_CLUSTERS - empty_clusters}/{K_CLUSTERS} clusters populated. {empty_clusters} empty clusters will use Global Fallback.")
    
    with open(os.path.join(RESULTS_DIR, f"ef_tables_local_k{K_CLUSTERS}.json"), "w") as f_json:
        json.dump(local_tables_dump, f_json, indent=4)

    # -------------------------------------------------------------------
    # 3. Online Evaluation (Local Tables)
    # -------------------------------------------------------------------
    print(f"  Running Online Evaluation (Local Mean)...")
    r_mean = eval_cluster_aware_local(f"Ours-Local (K={K_CLUSTERS}, Mean)", K_CLUSTERS, centroids, cluster_bins, global_list_mean, local_lists_mean)
    print(f"  R={r_mean['mean_r']:.4f}")
    all_results.append(r_mean)

    print(f"  Running Online Evaluation (Local P90)...")
    r_p90 = eval_cluster_aware_local(f"Ours-Local (K={K_CLUSTERS}, P90)", K_CLUSTERS, centroids, cluster_bins, global_list_p90, local_lists_p90)
    print(f"  R={r_p90['mean_r']:.4f}")
    all_results.append(r_p90)

    print(f"  Running Online Evaluation (Local P70)...")
    r_p70 = eval_cluster_aware_local(f"Ours-Local (K={K_CLUSTERS}, P70)", K_CLUSTERS, centroids, cluster_bins, global_list_p70, local_lists_p70)
