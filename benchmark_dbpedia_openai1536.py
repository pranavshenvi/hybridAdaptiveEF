...
Dataset: dbpedia-openai-1000k-angular (1M base vectors, 1536-dim, DBpedia
entity descriptions embedded with OpenAI text-embedding-ada-002 -- the SAME
embedding model the Ada-ef paper's own MS MARCO V1 dataset uses, which has
been blocked since 2026-09-16 on a Google Drive query-file bundle rate limit
(updateAsOf160926.md section 2). This gives a real read on the paper's exact
highest-dimension embedding family without needing that blocked download.

Picked deliberately, not just "another dataset": every text-retrieval corpus
tested so far (MS MARCO 3.22x, Cohere 2.18x) has shown wider difficulty
spread than every image/descriptor corpus tested (GloVe 1.86x, LAION
1.5-1.6x, SIFT 1.5-1.7x, DeepImage 1.18x) -- see summary.md section 7's
predictive framework. Not a proof (no cheap spread predictor has survived
testing -- see check_spread_proxy.py / check_lid_spread_proxy.py), but a
real, evidence-grounded bet that another text corpus is the best next shot
at a MS MARCO-style clean win. OpenAI's ada-002 also has no Cohere-style
separate query/document input_type modes, so this should land as symmetric
-- avoiding Cohere's calibration-collapse mechanism entirely.

Hosted as plain HDF5, same ann-benchmarks family as GloVe/DeepImage/SIFT,
but note: the primary ann-benchmarks.com host currently 403s on this
specific file (github.com/erikbern/ann-benchmarks/issues/460, open as of
this writing) -- use the Google Cloud Storage mirror instead. Verify the
mirror resolves before committing to the full download (~6GB at 1536-dim
float32 x 1M rows).

Not one of the Ada-ef paper's own 6 datasets; grouped with GloVe/DeepImage/
SIFT's K=100 protocol (page 14: "the remaining datasets from the ANNS
benchmark suite... K=100") since it's the same kind of ann-benchmarks-suite
entry, not one of the paper's own named datasets.
...
Download (server-side, before running this script):
  curl -I https://storage.googleapis.com/ann-datasets/ann-benchmarks/dbpedia-openai-1000k-angular.hdf5
  # if that returns 200:
  wget -O dbpedia-openai-1000k-angular.hdf5 https://storage.googleapis.com/ann-datasets/ann-benchmarks/dbpedia-openai-1000k-angular.hdf5
"""
...
RESULTS_DIR = f"results_dbpedia_openai1536_{TIMESTAMP}"
os.makedirs(RESULTS_DIR, exist_ok=True)

sys.stdout.reconfigure(encoding='utf-8')

class Logger(object):
    def __init__(self, filename=os.path.join(RESULTS_DIR, "benchmark_dbpedia_openai1536_sweep.log")):
...
# ═══════════════════════════════════════════════════════════════════════
#  Configuration -- K=100 group (page 14), matches GloVe/DeepImage/SIFT
# ═══════════════════════════════════════════════════════════════════════
K_SEARCH       = 100
TARGET_RECALL  = 0.95
...
# ═══════════════════════════════════════════════════════════════════════
print("═" * 80)
print("  Loading dbpedia-openai-1000k-angular Dataset")
print("═" * 80)
...
print("  Normalizing vectors (required for AdaEfPaperScorer's CosineDistanceEstimator; "
      "should already be near-unit-norm since this is an angular-metric dataset)...")
...
gt_path = f"ground_truth_dbpedia_openai1536_k{K_SEARCH}.npz"
...
# ═══════════════════════════════════════════════════════════════════════
index_path = "dbpedia_openai1536_efc500_m16.index"
...
minef_cache_path = f"dbpedia_openai1536_calib_min_ef_{N_CALIB}q_k{K_SEARCH}.npz"
...
ada_table_exact, WAE = load_or_build_target_recall_table(
    "cache_target_recall_ada_paper_dbpedia_openai1536.json", ada_scores_int, calib_q, calib_gt)
...
selfsamp_gt_path = f"dbpedia_openai1536_selfsamp_gt_{SAMPLE_SIZE}q.npz"
...
ada_table_selfsamp, WAE_selfsamp = load_or_build_target_recall_table(
    f"cache_target_recall_ada_paper_dbpedia_openai1536_selfsamp_n{SAMPLE_SIZE}.json",
...
for K_CLUSTERS in K_SWEEP:
    ...
    print(f"{'─' * 80}")
    cache_file = f"kmeans_cache_k{K_CLUSTERS}_dbpedia_openai1536.pkl"
...
# ═══════════════════════════════════════════════════════════════════════
print(f"\n{'═' * 80}")
print(f"  FINAL RESULTS -- dbpedia-openai-1000k-angular (target recall = {TARGET_RECALL}, n_test={n_test})")
print(f"{'═' * 80}\n")
