...
Dataset: GloVe (1.18M base vectors, 100d embeddings)
"""
...
RESULTS_DIR = f"results_glove_{TIMESTAMP}"
os.makedirs(RESULTS_DIR, exist_ok=True)

sys.stdout.reconfigure(encoding='utf-8')

class Logger(object):
    def __init__(self, filename=os.path.join(RESULTS_DIR, "benchmark_glove_exact_sweep.log")):
...
# effect can be isolated from the dataset itself, matching the same control
# added to the MS MARCO script.
SAMPLE_SIZE    = 200
...
# ═══════════════════════════════════════════════════════════════════════
print("═" * 80)
print("  Loading GloVe 100d Dataset (Normalized for L2)")
print("═" * 80)
...
gt_path = f"glove_ground_truth_{N_CALIB}kq.npz"
...
ada_table_exact, WAE = load_or_build_target_recall_table(
    "cache_target_recall_ada_paper_glove.json", ada_scores_int, calib_q, calib_gt)
...
# Their real Sketch (unmodified), built from this table. No WAE floor applied
# here -- their shipped Sketch class doesn't implement max(ef, WAE) either.
...
#  Sec 5.5 protocol: SAMPLE_SIZE points drawn from the corpus, used as proxy
#  queries) -- run alongside the real-query-calibrated row above so the
#  calibration-protocol effect can be isolated from the dataset itself.
# ═══════════════════════════════════════════════════════════════════════
print(f"\n{'═' * 80}")
...
selfsamp_gt_path = f"glove_selfsamp_gt_{SAMPLE_SIZE}q.npz"
...
ada_table_selfsamp, WAE_selfsamp = load_or_build_target_recall_table(
    f"cache_target_recall_ada_paper_glove_selfsamp_n{SAMPLE_SIZE}.json",
