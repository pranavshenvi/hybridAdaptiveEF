...
Dataset: Spotify 490K Audio Embeddings
"""
...
RESULTS_DIR = f"results_spotify_{TIMESTAMP}"
os.makedirs(RESULTS_DIR, exist_ok=True)

sys.stdout.reconfigure(encoding='utf-8')

class Logger(object):
    def __init__(self, filename=os.path.join(RESULTS_DIR, "benchmark_spotify_sweep.log")):
...
#  Configuration
# ═══════════════════════════════════════════════════════════════════════
EMBEDDING_FILE = 'vggish_embeddings.npz'
CORPUS_SIZE    = 480000
N_CALIB        = 3000
TEST_SIZE      = 3000
...
def compute_ground_truth(corpus_data, queries, k=10):
    n_queries = len(queries)
    gt = np.zeros((n_queries, k), dtype=np.int32)
    batch_size = 500
    for i in range(0, n_queries, batch_size):
        end = min(i + batch_size, n_queries)
        batch_dists = cdist(queries[i:end], corpus_data, metric='sqeuclidean')
        gt[i:end] = np.argsort(batch_dists, axis=1)[:, :k]
    return gt
...
# ═══════════════════════════════════════════════════════════════════════
print("═" * 80)
print(f"  Loading Spotify Dataset ({EMBEDDING_FILE})")
print("═" * 80)

data = np.load(EMBEDDING_FILE)
all_embeddings = data['features'].astype(np.float32)

norms = np.linalg.norm(all_embeddings, axis=1, keepdims=True)
all_embeddings = all_embeddings / (norms + 1e-9)

corpus = all_embeddings[:CORPUS_SIZE]
calib_q = all_embeddings[CORPUS_SIZE : CORPUS_SIZE + N_CALIB]
test_q = all_embeddings[-TEST_SIZE:]

dim = corpus.shape[1]
print(f"  Corpus: {corpus.shape} | Calib Q: {calib_q.shape} | "
...
print(f"  Corpus vector norm check (first 1000): mean={_norms.mean():.4f}, std={_norms.std():.4f} "
      f"(should be ~1.0 -- already normalized above; AdaEfPaperScorer's CosineDistanceEstimator "
      f"assumes unit-norm input, distribution.h line ~303)")
print(f"  Cluster Sweep Params: K_SWEEP={K_SWEEP}")

gt_path = "ground_truth_spotify.npz"
if os.path.exists(gt_path):
    ...
    print(f"\nComputing ground truth (chunked to prevent OOM)...")
    t0 = time.time()
...
#  HNSW Index
# ═══════════════════════════════════════════════════════════════════════
index_path = f"custom_spotify_{dim}d.index"
...
ada_table_exact, WAE = load_or_build_target_recall_table(
    "cache_target_recall_ada_paper_spotify.json", ada_scores_int, calib_q, calib_gt)
...
selfsamp_idx_arr = np.random.choice(len(corpus), SAMPLE_SIZE, replace=False)
selfsamp_q = corpus[selfsamp_idx_arr]

selfsamp_gt_path = f"spotify_selfsamp_gt_{SAMPLE_SIZE}q.npz"
...
ada_table_selfsamp, WAE_selfsamp = load_or_build_target_recall_table(
    f"cache_target_recall_ada_paper_spotify_selfsamp_n{SAMPLE_SIZE}.json",
...
for K_CLUSTERS in K_SWEEP:
    ...
    print(f"{'─' * 80}")
    cache_file = f"kmeans_cache_k{K_CLUSTERS}_spotify.pkl"
